...
if intregpar.warpf2use
    warp_arg = [warp_arg, ' --jacobian-constraint-weight ', intregpar.J, ...
        ...
        ' --inverse-consistency-weight ', intregpar.I];   
else
...
% test if registration file already exists
file_t1 = [oIm, filesep, 'registration'];
file_t2 = [strrep(oIm, 'j0', ''), filesep, 'registration'];
exist_status = exist(fullfile(oIm), 'file') == 2 || ...
    exist(fullfile(file_t1), 'file') == 2 || ...
    exist(fullfile([file_t1 '.gz']), 'file') == 2 || ...
    exist(fullfile(file_t2), 'file') == 2 || ...
    exist(fullfile([file_t2 '.gz']), 'file') == 2;

if ~exist_status || redo
